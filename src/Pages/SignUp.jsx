...
const SignUp = () => {
    ...
    const handleSignUp = async () => {
        try {
            const result = await createNewUser(auth, formDetails.email, formDetails.password);
            const user = auth.currentUser;
            if (user) {
                await updateProfile(user, {
                    displayName: formDetails.username,
                });
                    ...
                    <div className='w-[80%] h-[600px] flex flex-col items-center' >
                        <div className=' w-[90%]  min-h-full flex flex-col gap-2'>
                            ...
                            {/* <div className='flex gap-3 mb-7 '>
                                <Button variant="outlined" className='' style={{ textTransform: 'none', fontSize: "14px", color: "black" }}>
                            ...
                            <div className='flex flex-col gap-5'>
                                <TextField label="Username" variant="standard" value={username} type='text' onChange={(e) => setUsername(e.target.value)} />
                                <TextField label="Email" variant="standard" value={email} type='email' onChange={(e) => setEmail(e.target.value)} />
                                <TextField label="Password" variant="standard" value={password} type='password' onChange={(e) => setPassword(e.target.value)} />
                                <TextField label="ConfirmPassword" variant="standard" value={confirmPassword} type='password' onChange={(e) => setConfirmPassword(e.target.value)} />
                                <TextField label="PhoneNumber" variant="standard" value={number} type='number' onChange={(e) => setNumber(e.target.value)} />
                                <CustomButtom title="Sign Up" handlePress={handleSignUp} containerStyles="bg-black text-white w-[90%] shadow-2xl py-3 rounded-[8px] self-center mt-2" textStyles=" text-sm text-center font-poppins " />
                                <CustomButtom title="Sign In" handlePress={() => navigate("/SignIn")} containerStyles="bg-white text-[#5B86E5] w-[90%] shadow-2xl py-3 rounded-[8px] border-2 border-[#5B86E5] self-center" textStyles=" text-sm text-center font-poppins " />
                            </div>
                        </div>
                    </div>
                </div>
